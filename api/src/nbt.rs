...
pub fn parse_item_name(input: &str) -> Result<String, Error> {
	let item: item::Item = parse_from_encoded(input)?;
	let item = item.i.into_iter().next().ok_or(Error::InvalidNbt)?;
...
pub fn parse_from_encoded<'i, T>(input: &'i str) -> Result<T, Error>
where
	T: Deserialize<'i>,
{
	let bytes = STANDARD.decode(input).map_err(|_| Error::InvalidBase64)?;
	let result = GzDecoder::new(&bytes[..]);

	fastnbt::from_reader(result).map_err(|_| Error::InvalidNbt)
}
