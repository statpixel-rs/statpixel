...
pub async fn begin(pool: &PostgresPool) -> Result<(), Error> {
	loop {
		...
		let guilds = guild_schedule::table
			...
			.get_results::<(Uuid, DateTime<Utc>, i64)>(&mut connection)?;

		if guilds.is_empty() {
			// Sleep for 3 hours, since that's the earliest a profile could be added.
			tokio::time::sleep(tokio::time::Duration::from_secs(60 * 60)).await;

			continue;
		}

		info!(snapshots = guilds.len(), "updating guild snapshots");

		futures::stream::iter(guilds)
...
pub fn get_or_insert(
	...
	if let Some(snapshot) = get(ctx, guild, timeframe)? {
...
pub fn insert(ctx: Context<'_>, guild: &Guild) -> Result<(), Error> {
	let encoded = encode(guild)?;
