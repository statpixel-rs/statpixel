...
showing-statistics = <a:clock:1113624864272683060> Показ статистики зміни з {$from} до {$to}.
...
showing-comparison = <a:clock:1113624864272683060> Порівняння статистики **`{$from}`** з **`{$to}`**.

# Помилки

error-player-not-found = Профіль, належний **`{$name}`**, не знайдено.
error-snapshot-not-found = Сесію, належну **`{$name}`**, не знайдено.
...
error-leaderboard-player-not-found = Наданий гравець не був знайдений у перших або останніх 15 000 позиціях лідерборду.
...
track-success-title = Відстеження гравця {$name}
...
help-history = Історія <a:clock:1113624864272683060>
...
linking-failed-username-description = Ім'я користувача **`{$name}`** не належить обліковому запису Minecraft.
...
showing-statistics-at = <a:clock:1113624864272683060> Показ статистики з {$at}.
...
error-session-already-exists = Сесія з вказаним ім'ям вже була створена вами.

session-deleted = Сесію з ідентифікатором **`{$id}`** було видалено.
session-created-title = Створено сесію
session-created = Сесію з ідентифікатором **`{$id}`** було створено для **`{$name}`**.
  Підставте ідентифікатор сесії в будь-яку команду, щоб використовувати її дані.

  Спробуйте це: </bedwars general:1113624864272683060>

  Або використайте команду session для перевірки прогресу гравця з моменту початку сесії: </bedwars session:1113624864272683060>
  Зверніть увагу, що ця команда - це скорочення для </bedwars compare:1113624864272683060>!
